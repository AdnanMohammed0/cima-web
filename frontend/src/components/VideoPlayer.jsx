...
export default function VideoPlayer({
  ...
  useEffect(() => {
    ...
    fetch(`${BACKEND_URL}/api/subtitles/search?tmdb_id=${mediaId}&lang=ar&type=${subType}${season ? `&season=${season}` : ''}${episode ? `&episode=${episode}` : ''}`).then(r => r.json()).then(data => {
      if (data.subtitles && data.subtitles.length > 0) {
        const best = data.subtitles[0];
        fetch(`${BACKEND_URL}/api/subtitles/download?id=${best.id}`).then(r => r.text()).then(vtt => {
  ...
  const loadAvailableSubs = async () => {
    ...
    try {
      const subType = mediaType === 'tv' ? 'tv' : 'movie';
      const resp = await fetch(`${BACKEND_URL}/api/subtitles/search?tmdb_id=${mediaId}&lang=ar&type=${subType}${season ? `&season=${season}` : ''}${episode ? `&episode=${episode}` : ''}`);
  ...
  const loadArabicSubs = async () => {
    ...
    try {
      ...
      for (const sub of data.subtitles) {
        try {
          const dl = await fetch(`${BACKEND_URL}/api/subtitles/download?id=${sub.id}`);
          if (!dl.ok) continue;
                      ...
                      <div className="border-t border-white/5 pt-1 max-h-40 overflow-y-auto">
                        ...
                        {availableSubs.map((sub, i) => (
                          <button key={i} onClick={async () => {
                            try {
                              const r = await fetch(`${BACKEND_URL}/api/subtitles/download?id=${sub.id}`);
